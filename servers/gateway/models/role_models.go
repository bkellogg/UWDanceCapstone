...
func (nr *NewRole) Validate() error {
	if len(nr.Name) > appvars.MaxRoleNameLength {
		return fmt.Errorf("role name can only be %s characters long", appvars.MaxRoleNameLength)
	}
	nr.Name = strings.ToLower(nr.Name)
	if len(nr.DisplayName) > appvars.MaxRoleDisplayNameLength {
		return fmt.Errorf("role display name can only be %s characters long", appvars.MaxRoleDisplayNameLength)
	}
	nr.DisplayName = strings.ToTitle(nr.DisplayName)
